...
export class PagingComponent {
  ...
  public get pages(): number[] {
    ...
    if (this.currentPage > 1) {
      pages.push(this.currentPage - 1); // adds the previous page
    }

    pages.push(this.currentPage); // adds the current page

    if (this.currentPage < this.totalPages) {
      pages.push(this.currentPage + 1); // adds the next page
    }
    ...
    if (this.currentPage + 2 < this.totalPages && this.currentPage === 1) {
      pages.push(this.currentPage + 3); // this is a hack for some cases :D
    }
